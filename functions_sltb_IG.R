...
laplace_sltb_mixed_common_fast <- function(data,
  ...
  update_tau_map_numeric <- function(u_vec, a1, b1) {
    G_local <- length(u_vec)
    num <- 0.5 * sum(u_vec^2) + b1
    den <- (G_local / 2) + a1 + 1
    tau_hat_local <- as.numeric(num / den)
  ...
  find_map_scaled <- function(y_local, ll_scale = 1) {
    ...
    while (iterloc < max_iter) {
      ...
      tau_new <- update_tau_map_numeric(up$u, a1 = a1, b1 = b1)
  ...
  compute_laplace_scaled <- function(y_local, ll_scale = 1) {
    ...
    tryCatch({
      ...
      A_tau <- Su2 + b1
      B_tau <- G / 2 + a1 + 1
      H_tautau <- 2 * A_tau / (tau_hat^3) - B_tau / (tau_hat^2)
